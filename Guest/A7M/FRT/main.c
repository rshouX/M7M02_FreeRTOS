...
#define SEM_CNT_MAX             (32U)
#define QUEUE_LENGTH            (32U)
...
void Test_Yield_1(void)
{
    ...
    for(Count=0;Count<ROUND_NUM;Count++)
    {
        ...
        if(Flip!=0U)
        {
            RVM_DBG_S("Yield error in Thd1 @ round ");
            RVM_DBG_I(Count);
            RVM_DBG_S(".\r\n");
        }
        Flip=1U;
...
void Test_Yield_2(void)
{
    ...
    for(Count=0;Count<ROUND_NUM;Count++)
    {
        ...
        if(Flip==0U)
        {
            RVM_DBG_S("Yield error in Thd2 @ round ");
            RVM_DBG_I(Count);
            RVM_DBG_S(", ");
            RVM_DBG_I(Start);
            RVM_DBG_S(", ");
...
void Test_Mem_Pool(void)
{
    ...
    for(Test_Cnt=0;Test_Cnt<ROUND_NUM;Test_Cnt++)
    {
        ...
        if(Mem[Alloc[0]]==NULL) RVM_DBG_S("Failure.\r\n");
...
void Int_Handler(void)
{
    ...
    else if(Count<ROUND_NUM*2U)
    {
        ...
        portYIELD_FROM_ISR(Task_Woke);
        if(Retval!=pdTRUE)
            RVM_DBG_S("ISR semaphore post failed.\r\n");
    }
    else if(Count<ROUND_NUM*3U)
    {
        ...
        portYIELD_FROM_ISR(Task_Woke);
        if(Retval!=pdTRUE)
            RVM_DBG_S("ISR bmq message send failed.\r\n");
    }
    else
    {
        Retval=0;
